...
export function packRoutes() {
  ...
  app.get("/", async (c) => {
    ...
    let sql = `
      SELECT id, creator_user_id, title, description, moderation_status,
             zip_asset_id, thumbnail_asset_id, published_at, created_at, updated_at
      FROM packs
  ...
  app.post("/", requireAuth, async (c) => {
    ...
    await c.env.DB
      .prepare(
        `INSERT INTO packs (
          id, creator_user_id, title, description, moderation_status,
          zip_asset_id, thumbnail_asset_id, created_at, updated_at
        ) VALUES (?, ?, ?, ?, 'draft', ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`
      )
      .bind(packId, auth.userId, title, body?.description?.trim() ?? null, body?.zipAssetId?.trim() ?? null, body?.thumbnailAssetId?.trim() ?? null)
      .run();
  ...
  app.patch("/:packId", requireAuth, async (c) => {
    ...
    await c.env.DB
      ...
      .bind(
        ...
        body.zipAssetId !== undefined ? body.zipAssetId?.trim() ?? null : existing.zip_asset_id,
  ...
  app.get("/:packId/download", optionalAuth, async (c) => {
    ...
    const pack = await c.env.DB
      .prepare("SELECT id, creator_user_id, title, moderation_status, zip_asset_id FROM packs WHERE id = ?")
      .bind(packId)
      .first<{
        id: string;
        creator_user_id: string;
